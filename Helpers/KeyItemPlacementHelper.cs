...
This process is done once for the combined `Kmoon` and/or `Ksummon` determination, and then once each for each `Kmiab` area. Should a `Kmiab` area be given more slots than it has boxes, all the boxes are added to the pool."
        )
