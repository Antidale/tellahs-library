...
using tellahs_library.TournamentCommands.Helpers;

namespace tellahs_library.TournamentCommands
{
    [Command("Tournament")]
    ...
    public class Tournament(FeInfoHttpClient client)
    {
        ...
        )
        {
            try
            {
                ...
                var responseDto = (await response.Content.ReadFromJsonAsync<ChangeRegistrationResponse>()) ?? new ChangeRegistrationResponse(0, 0, 0, 0, 0, "");
                //respond to the user
                await ctx.EditResponseAsync(string.Join(separator: ' ',
                        "You're no longer registered",
                        responseDto.TournamentName.HasContent()
                            ? $"in {responseDto.TournamentName}"
                            : null));
        ...
        private static string MapNoneEntries(string userInput)
        {
