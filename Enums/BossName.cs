using System.ComponentModel;

namespace tellahs_library.Enums
{
    public enum BossName
    {
        [Description("Unknown Boss")]
        Unknown,
        [Description("D.Mist")]
        DMist,
        [Description("Kaipo Guards")]
        KaipoGuards,
        [Description("Octomamm")]
        Octomamm,
        [Description("Antlion")]
        Antlion,
        [Description("Waterhag")] Waterhag,
        [Description("MomBomb")]
        MomBomb,
        [Description("Fabul Gauntlet")]
        FabulGauntlet,
        [Description("Milon")]
        Milon,
        [Description("Milon Z.")]
        MilonZ,
        [Description("DKC")]
        DarkKnightCecil,
        [Description("Baron Guards")]
        BaronGuards,
        [Description("Karate")]
        Karate,
        [Description("Baigan")]
        Baigan,
        [Description("Kainazzo")]
        Kainazzo,
        [Description("Dark Elf")]
        DarkElf,
        [Description("Magus Sisters")]
        MagusSisters,
        [Description("Valvalis")]
        Valvalis,
        [Description("Calbrena")]
        Calbrena,
        [Description("Golbez")]
        Golbez,
        [Description("Dr Lugae")]
        DrLugae,
        [Description("Dark Imps")]
        DarkImps,
        [Description("K/Q Eblan")]
        KingQueenEblan,
        [Description("Rubicant")]
        Rubicant,
        [Description("Evil Wall")]
        EvilWall,
        [Description("Elements")]
        Elements,
        [Description("CPU")]
        CPU,
        [Description("Odin")]
        Odin,
        [Description("Asura")]
        Asura, 
        [Description("Leviatan")]
        Leviatan,
        [Description("Bahamut")] Bahamut,
        [Description("Pale Dim")] PaleDim,
        [Description("Wyvern")] Wyvern,
        [Description("Plague")] Plague,
        [Description("D.Lunars")] DLunars,
        [Description("Ogopogo")] Ogopogo,
        [Description("Zeromus")] Zeromus,
    }
}
